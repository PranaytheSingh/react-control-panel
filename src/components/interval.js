...
function Range(root, opts, theme, uuid) {
  ...
  if (!!opts.step && !!opts.steps) {
    throw new Error(
      'Cannot specify both step and steps. Got step = ' + opts.step + ', steps = ',
      opts.steps
    );
  }
  ...
  if (opts.scale === 'log') {
    ...
    opts.initial = [
      scaleValueInverse(
        isnumeric(opts.initial[0])
          ? opts.initial[0]
          : scaleValue(opts.min + (opts.max - opts.min) * 0.25)
      ),
      scaleValueInverse(
        isnumeric(opts.initial[1])
          ? opts.initial[1]
          : scaleValue(opts.min + (opts.max - opts.min) * 0.75)
      ),
    ];

    if (
      scaleValue(opts.initial[0]) * scaleValue(opts.max) <= 0 ||
      scaleValue(opts.initial[1]) * scaleValue(opts.max) <= 0
    ) {
      throw new Error(
        'Log range initial value must have the same sign as min/max and must not equal zero. Got initial value = [' +
          scaleValue(opts.initial[0]) +
          ', ' +
          scaleValue(opts.initial[1]) +
          ']'
      );
    }
  } else {
  ...
  function setActiveValue(fraction) {
    ...
    var newValue =
      opts.min + Math.round(((opts.max - opts.min) * fraction) / opts.step) * opts.step;
